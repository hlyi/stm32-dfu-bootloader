## to build non-standard bluepill CH32F103 board with USBC, most likely use the following options:
# make STD_BLUEPILL_PIN=0 USB_PULLUP_FIX=1
## to build standard  bluepill CH32F103 board
# make
...
ifeq ($(STD_BLUEPILL_PIN),0)
# nonstarnd bluepill pin, i.e. BOOT1 is always pulldown to zero. We need define custom pin to enter boot loader, default is PB11
PIN_OPTS	+= -DGPIO_DFU_BOOT_PIN=11
else
# standand bluepill pin, i.e. BOOT1 = PB2, it has 100k resistor either pull up or down
PIN_OPTS	+= -DGPIO_DFU_BOOT_PIN=2 -DGPIO_DFU_BOOT_PIN_NOPD
endif

# some board has wrong (weak) pull up resistor, enable CH32F103 internal pullup resister
ifeq ($(USB_PULLUP_FIX),1)
PIN_OPTS	+= -DENABLE_USB_INT_PULLUP
endif

ifneq ($(CH32F103),0)
EXTRA_BUILD_OPTS	+= -DENABLE_CH32F103
endif

ifeq ($(USE_BACKUP_REGS),1)
EXTRA_BUILD_OPTS	+= -DUSE_BACKUP_REGS
endif

# Default config
#CONFIG ?= -DWINUSB_SUPPORT -DENABLE_CHECKSUM -DENABLE_WATCHDOG=20
CONFIG ?= -DWINUSB_SUPPORT -DENABLE_GPIO_DFU_BOOT -DGPIO_DFU_BOOT_PORT=GPIOB $(PIN_OPTS) -DENABLE_DFU_UPLOAD -DENABLE_LED_STATUS -DGPIO_LED_STATUS_PORT=GPIOC -DGPIO_LED_STATUS_PIN=13 -DENABLE_SHORT_POLL $(EXTRA_BUILD_OPTS)
